...
var Graphics$1 = function Graphics(root, _ref) {
  ...
  invariant(!geometry || geometry instanceof graphics.Graphics, "Graphics geometry needs to be a `Graphics`");
...
var AnimatedSprite$1 = function AnimatedSprite(root, props) {
  ...
  var makeTexture = function makeTexture(textures) {
    return textures.map(function (texture) {
      invariant(texture instanceof core.Texture || (texture === null || texture === void 0 ? void 0 : texture.texture), "AnimationSprite texture needs to be an array of `Texture` or `{ texture: Texture, time:\n                number }`");
      return texture;
    });
  };
...
function unmountComponent(container) {
  invariant(display.Container.prototype.isPrototypeOf(container), 'Invalid argument `container`, expected instance of `Container`.');
...
function useApp() {
  ...
  invariant(app$1 instanceof app.Application, 'No Context found with `%s`. Make sure to wrap component with `%s`', 'Application', 'AppProvider');
  return app$1;
}
